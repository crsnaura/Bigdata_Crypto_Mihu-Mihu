...
def load_core_data():
    try:
        price = pd.read_csv("data/hourly_price.csv", parse_dates=["timestamp"])
        cum = pd.read_csv("data/cum_return.csv", parse_dates=["timestamp"])
        log_ret = pd.read_csv("data/log_return.csv", parse_dates=["timestamp"])
        raw_data = pd.read_csv("data/crypto_data.csv")
        return price, cum, log_ret, raw_data
    except Exception as e:
        st.error(f"Pipeline data belum siap. Jalankan pipeline data analitik terlebih dahulu! Error: {e}")
        st.stop()
